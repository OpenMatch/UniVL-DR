...
if __name__ == '__main__':
    ...
    if args.txt_embed_path:
        ...
        for step, qid in enumerate(query_idx):
            ...
            pos_ids = set(instance['txt_posFacts'])
            neg_ids = []
            for idx in I[step]:
                real_idx = txt_idx[idx]
                if real_idx not in pos_ids:
                    neg_ids.append(real_idx)
                if len(neg_ids) > args.max_neg:
                    break
            data_dict[qid]['txt_negFacts'] = neg_ids
        del cpu_index
    ...
    if args.img_embed_path:
        ...
        for step, qid in enumerate(query_idx):
            instance = data_dict[qid]
            pos_ids = set(instance['img_posFacts'])
            neg_ids = []
            for idx in I[step]:
                real_idx = img_idx[idx]
                if real_idx not in pos_ids:
                    neg_ids.append(real_idx)
                if len(neg_ids) > args.max_neg:
                    break
            data_dict[qid]['img_negFacts'] = neg_ids
        del cpu_index
